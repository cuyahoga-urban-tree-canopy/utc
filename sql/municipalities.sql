...
SELECT cartodb_id , replace(municipa_4, ' ', E'\n') AS name, percent_canopy, the_geom_webmercator FROM utc_summarized_all WHERE "municipa_4" <> 'Cleveland'

UNION

SELECT cartodb_id, replace(spa_name, ' ', E'\n')  AS name, percent_canopy, the_geom_webmercator FROM utc_summarized_all WHERE "type" = 'ClevelandSPA'
